# PSAddonBYFlow

Аддон для [ProtectionStones](https://github.com/espidev/ProtectionStones), добавляющий систему прочности приватам на Paper 1.21.1. Плагин хранит количество "жизней" для каждого региона, показывает их в голограмме и защищает приватные блоки от типичных эксплойтов.

## Возможности
- Настраиваемое количество жизней, урон от взрыва и смещение голограммы для каждого материала привата.
- Многострочные голограммы с плейсхолдерами `{lives}`, `{max}`, `{owner}`, координатами и HEX‑цветами.
- Автоматическое удаление голограмм при разрушении привата и поддержка плагина DecentHolograms.
- Защита от установки приватных блоков вплотную друг к другу и от разрушения визером.
- Настраиваемые сообщения об ошибках при попытке наложить приват.

## Сборка
1. Установите JDK 21. Gradle Wrapper уже добавлен, поэтому достаточно выполнить `./gradlew :region-addon:build`.
2. Готовый файл появится в `region-addon/build/libs/PSAddonRegions-{version}.jar`.
3. Скопируйте `PSAddonRegions` и оригинальный ProtectionStones в папку `plugins` сервера Paper 1.21.1 и перезапустите сервер.

## Конфигурация `plugins/PSAddonRegions/config.yml`
```yaml
default:
  lives: 3
  damage-per-explosion: 1
  tnt-only: true
  hologram:
    enabled: true
    offset-y: 1.8
    lines:
      - "&cЖизни привата: &#ff5555{lives}&7/&f{max}"
      - "&7Владелец: &f{owner}"

blocks:
  DIAMOND_BLOCK:
    lives: 5
    hologram:
      lines:
        - "&bАлмазный приват"
        - "&f{lives}&7/&f{max} &8жизней"

prevent-stacking: true

messages:
  stack-block-denied: "&cНельзя ставить приват вплотную к другому приватному блоку!"
```

- Раздел `default` определяет настройки по умолчанию для всех приватных блоков. Раздел `blocks` позволяет переопределить значения для конкретных материалов.
- Если `tnt-only: true`, урон региону наносят только взрывы TNT. Любые другие взрывы игнорируются.
- После изменения конфигурации перезапустите сервер, чтобы настройки применились.

## Хранение данных
Файл `plugins/PSAddonRegions/data.yml` содержит текущие жизни приватных регионов. Его можно безопасно удалить для сброса состояния — плагин создаст его заново при следующем запуске.
