...
## Сборка
1. Установите JDK 21. Gradle Wrapper уже добавлен, поэтому достаточно выполнить `./gradlew build`.
2. Готовый файл появится в `build/libs/PSAddonBYFlow-{version}.jar`.
3. Скопируйте `PSAddonBYFlow` и оригинальный ProtectionStones в папку `plugins` сервера Paper 1.21.1 и перезапустите сервер.
